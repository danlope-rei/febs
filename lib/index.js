...
const runVunit = (cmd, spec) => vunit.run({
  spec: spec,
...
const runJsAndVue = (cmd) => {
  ...
  return runVunit(cmd, cmd.testDirGlob);
};

/**
 * Determine which test tool needed (either mocha or vunit)
 * @param cmd
 * @returns {string}
 */
const onlyJsTests = cmd => {

  const files = cmd.testDirGlob.includes('*') ?
      glob.sync(path.join(cwd, cmd.testDirGlob)) :
      glob.sync(path.join(cwd, cmd.testDirGlob, '**', '*-spec.js'));
